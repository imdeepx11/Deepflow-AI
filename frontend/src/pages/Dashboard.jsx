    ...
    <div className="p-8 space-y-6 max-w-[1400px] mx-auto bg-[#F8FAFC] dark:bg-[#050505] transition-colors min-h-screen">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            ...
            <tbody className="divide-y divide-[#E2E8F0] dark:divide-[#242424]">
              {documents.map((doc) => {
                const confPercent = Math.round((doc.confidence ?? doc.ai_confidence ?? 0.95) * 100);
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#0D0D0D] border border-[#E2E8F0] dark:border-[#242424] rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-xl">
            <h3 className="text-base font-bold text-[#0F172A] dark:text-[#F5F5F5]">Delete Document</h3>
            <p className="text-xs text-[#64748B] dark:text-[#A1A1AA]">
              Are you sure you want to delete <strong className="text-[#0F172A] dark:text-white">{deleteConfirm.name}</strong>? This action cannot be undone.
            </p>
            <div className="flex justify-end gap-3 pt-2">
              <button
                onClick={() => setDeleteConfirm(null)}
                className="px-4 py-2 text-xs font-semibold text-[#64748B] dark:text-[#A1A1AA] hover:bg-[#F1F5F9] dark:hover:bg-[#1A1A1A] rounded-lg transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                className="px-4 py-2 text-xs font-bold bg-[#EF4444] hover:bg-[#DC2626] text-white rounded-lg transition-colors"
              >
                Delete
              </button>
            </div>
      ...
      <div className="pt-4 border-t border-[#E2E8F0] dark:border-[#242424] flex items-center justify-between text-xs text-[#94A3B8] dark:text-[#64748B]">
        <div className="flex items-center gap-1.5 font-medium">
          <span>Enterprise Intelligent Document Ingestion</span>
        </div>
